// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.31.1
// source: queries.sql

package exercisesdb

import (
	"context"

	"github.com/google/uuid"
)

const countExercises = `-- name: CountExercises :one
SELECT count(*) FROM exercises
WHERE ($1::text = '' OR name ILIKE '%' || $1::text || '%')
  AND ($2::text = '' OR $2::text = ANY(primary_muscles) OR $2::text = ANY(secondary_muscles))
  AND ($3::text = '' OR category = $3::text)
  AND (cardinality($4::text[]) = 0 OR equipment = ANY($4::text[]))
`

type CountExercisesParams struct {
	Query     string
	Muscle    string
	Category  string
	Equipment []string
}

// CountExercises mirrors SearchExercises' filters so the browse page can say
// how many matched without paging through them.
func (q *Queries) CountExercises(ctx context.Context, arg CountExercisesParams) (int64, error) {
	row := q.db.QueryRow(ctx, countExercises,
		arg.Query,
		arg.Muscle,
		arg.Category,
		arg.Equipment,
	)
	var count int64
	err := row.Scan(&count)
	return count, err
}

const getExercise = `-- name: GetExercise :one
SELECT id, slug, name, category, equipment, difficulty, instructions, video_url, primary_muscles, secondary_muscles, created_at, updated_at, illustration_slug FROM exercises WHERE id = $1
`

func (q *Queries) GetExercise(ctx context.Context, id uuid.UUID) (Exercise, error) {
	row := q.db.QueryRow(ctx, getExercise, id)
	var i Exercise
	err := row.Scan(
		&i.ID,
		&i.Slug,
		&i.Name,
		&i.Category,
		&i.Equipment,
		&i.Difficulty,
		&i.Instructions,
		&i.VideoUrl,
		&i.PrimaryMuscles,
		&i.SecondaryMuscles,
		&i.CreatedAt,
		&i.UpdatedAt,
		&i.IllustrationSlug,
	)
	return i, err
}

const getExerciseBySlug = `-- name: GetExerciseBySlug :one
SELECT id, slug, name, category, equipment, difficulty, instructions, video_url, primary_muscles, secondary_muscles, created_at, updated_at, illustration_slug FROM exercises WHERE slug = $1
`

func (q *Queries) GetExerciseBySlug(ctx context.Context, slug string) (Exercise, error) {
	row := q.db.QueryRow(ctx, getExerciseBySlug, slug)
	var i Exercise
	err := row.Scan(
		&i.ID,
		&i.Slug,
		&i.Name,
		&i.Category,
		&i.Equipment,
		&i.Difficulty,
		&i.Instructions,
		&i.VideoUrl,
		&i.PrimaryMuscles,
		&i.SecondaryMuscles,
		&i.CreatedAt,
		&i.UpdatedAt,
		&i.IllustrationSlug,
	)
	return i, err
}

const listExercisesBySlugs = `-- name: ListExercisesBySlugs :many
SELECT id, slug, name, category, equipment, difficulty, instructions, video_url, primary_muscles, secondary_muscles, created_at, updated_at, illustration_slug FROM exercises WHERE slug = ANY($1::text[])
`

func (q *Queries) ListExercisesBySlugs(ctx context.Context, slugs []string) ([]Exercise, error) {
	rows, err := q.db.Query(ctx, listExercisesBySlugs, slugs)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []Exercise{}
	for rows.Next() {
		var i Exercise
		if err := rows.Scan(
			&i.ID,
			&i.Slug,
			&i.Name,
			&i.Category,
			&i.Equipment,
			&i.Difficulty,
			&i.Instructions,
			&i.VideoUrl,
			&i.PrimaryMuscles,
			&i.SecondaryMuscles,
			&i.CreatedAt,
			&i.UpdatedAt,
			&i.IllustrationSlug,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const listExercisesForEquipment = `-- name: ListExercisesForEquipment :many
SELECT id, slug, name, category, equipment, difficulty, instructions, video_url, primary_muscles, secondary_muscles, created_at, updated_at, illustration_slug FROM exercises
WHERE equipment = ANY($1::text[])
ORDER BY category, name
LIMIT $2::int
`

type ListExercisesForEquipmentParams struct {
	Equipment   []string
	ResultLimit int32
}

// ListExercisesForEquipment feeds the plan generator's candidate list: every
// exercise someone with this equipment could actually perform, bodyweight
// included. Ordered so the list handed to the model is stable between runs.
func (q *Queries) ListExercisesForEquipment(ctx context.Context, arg ListExercisesForEquipmentParams) ([]Exercise, error) {
	rows, err := q.db.Query(ctx, listExercisesForEquipment, arg.Equipment, arg.ResultLimit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []Exercise{}
	for rows.Next() {
		var i Exercise
		if err := rows.Scan(
			&i.ID,
			&i.Slug,
			&i.Name,
			&i.Category,
			&i.Equipment,
			&i.Difficulty,
			&i.Instructions,
			&i.VideoUrl,
			&i.PrimaryMuscles,
			&i.SecondaryMuscles,
			&i.CreatedAt,
			&i.UpdatedAt,
			&i.IllustrationSlug,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const searchExercises = `-- name: SearchExercises :many
SELECT id, slug, name, category, equipment, difficulty, instructions, video_url, primary_muscles, secondary_muscles, created_at, updated_at, illustration_slug FROM exercises
WHERE ($1::text = '' OR name ILIKE '%' || $1::text || '%')
  AND ($2::text = '' OR $2::text = ANY(primary_muscles) OR $2::text = ANY(secondary_muscles))
  AND ($3::text = '' OR category = $3::text)
  AND (cardinality($4::text[]) = 0 OR equipment = ANY($4::text[]))
ORDER BY name
LIMIT $6::int OFFSET $5::int
`

type SearchExercisesParams struct {
	Query        string
	Muscle       string
	Category     string
	Equipment    []string
	ResultOffset int32
	ResultLimit  int32
}

// SearchExercises is the browse page's one query. Every filter is optional:
// an empty query, muscle, category, or equipment list means "no constraint",
// which keeps this to a single statement rather than a builder.
//
// Paged by offset rather than a keyset cursor. The ORDER BY is name, which is
// stable and unique enough here, and the catalog is 455 rows — an offset deep
// into it costs nothing a keyset would save. Page links also need to jump to an
// arbitrary page, which a cursor cannot do.
func (q *Queries) SearchExercises(ctx context.Context, arg SearchExercisesParams) ([]Exercise, error) {
	rows, err := q.db.Query(ctx, searchExercises,
		arg.Query,
		arg.Muscle,
		arg.Category,
		arg.Equipment,
		arg.ResultOffset,
		arg.ResultLimit,
	)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []Exercise{}
	for rows.Next() {
		var i Exercise
		if err := rows.Scan(
			&i.ID,
			&i.Slug,
			&i.Name,
			&i.Category,
			&i.Equipment,
			&i.Difficulty,
			&i.Instructions,
			&i.VideoUrl,
			&i.PrimaryMuscles,
			&i.SecondaryMuscles,
			&i.CreatedAt,
			&i.UpdatedAt,
			&i.IllustrationSlug,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const slugForIllustration = `-- name: SlugForIllustration :one
SELECT slug FROM exercises
WHERE illustration_slug = $1::text
ORDER BY (slug = $1::text) DESC, slug
LIMIT 1
`

// The catalogue exercise drawn by one artwork directory. Several can share
// one; the one named the same as the artwork wins, then the first by name.
func (q *Queries) SlugForIllustration(ctx context.Context, illustration string) (string, error) {
	row := q.db.QueryRow(ctx, slugForIllustration, illustration)
	var slug string
	err := row.Scan(&slug)
	return slug, err
}

const slugForVideo = `-- name: SlugForVideo :one
SELECT slug FROM exercises
WHERE video_url = $1::text
ORDER BY slug
LIMIT 1
`

// The catalogue exercise a demonstration video belongs to.
func (q *Queries) SlugForVideo(ctx context.Context, videoUrl string) (string, error) {
	row := q.db.QueryRow(ctx, slugForVideo, videoUrl)
	var slug string
	err := row.Scan(&slug)
	return slug, err
}
